...
func AdminApi(service *builder.Engine) {
	...
	authApi := service.Group("admin/api", middleware.AdminAuthMiddleware)
	{
		...
		authApi.Post("/userroles", (&controller.User{}).Roles)                  // 后台用户所绑定的角色
		authApi.Post("/user/menuTree", (&controller.User{}).MenuTree)           // 登录的用户所拥有的菜单列表树
		...
		authApi.Get("/role/option", (&controller.Role{}).Option)                 // 角色选项
		authApi.Get("/role/bindMenu", (&controller.Role{}).BindMenu)             // 角色绑定菜单
		authApi.Get("/role/menus", (&controller.Role{}).Menus)                   // 角色所绑定的菜单
		authApi.Get("/role/bindPermission", (&controller.Role{}).BindPermission) // 角色绑定权限
		authApi.Get("/role/permissions", (&controller.Role{}).Permissions)       // 角色所绑定的权限
